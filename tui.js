...
function setupBhagavadGitaReloadButton(button) {
  if (button == null) {
    return;
  }

  button.addEventListener("click", async () => {
    await displayRandomBhagavadGitaVerse();
  });
...
async function fetchRandomBhagavadGitaVerse() {
  const chapter = Math.floor(Math.random() * 18) + 1;
  const verse = Math.floor(Math.random() * 47) + 1; 
...
async function displayRandomBhagavadGitaVerse(parentElement = null) {
  try {
    const bhagavadGitaVerse =
      document.getElementsByClassName("bhagavad-gita-verse")?.[0] ||
      document.createElement("div");

    if (!bhagavadGitaVerse.classList.contains("bhagavad-gita-verse")) {
      bhagavadGitaVerse.classList.add("bhagavad-gita-verse");
    }
    ...
    textElement.innerHTML = "Connecting to the Bhagavad Gita...";

    const reloadButton = document.createElement("button");
    setupBhagavadGitaReloadButton(reloadButton);

    bhagavadGitaVerse.innerHTML = "";

    bhagavadGitaVerse.appendChild(reloadButton);
    bhagavadGitaVerse.appendChild(textElement);
    bhagavadGitaVerse.appendChild(referenceElement);

    parentElement?.appendChild(bhagavadGitaVerse);

    const { slok, chapter, verse } = await fetchRandomBhagavadGitaVerse();
    referenceElement.innerText = `Chapter ${chapter}, Verse ${verse}`;

    const wordsToHighlight = [
      "अनंत", "अर्जुन", "आर्यमन", "अश्वत्थामा", "भीष्म", 
      "ब्रह्मा", "बुद्ध", "धृतराष्ट्र", "द्रोण", "दुर्योधन", "गांडीव",
      "गरुड़", "इंद्र", "जनक", "कौरव", "कृष्ण", "माधव", "मनु", 
      "पांडव", "पृत्था", "राम", "संजय", "शिव", "विष्णु", "युधिष्ठिर"
    ];

    const regex = new RegExp(`\\b(${wordsToHighlight.join('|')})\\b`, 'gi');

    textElement.innerHTML = slok.replace(regex, `<span class="${getRandomTextColorClass()}">$1</span>`);
  } catch (e) {
    console.error(e);
  }
}
...
function colorizeCode() {
  ...
  const keywords = {
    ...
    mongo : javascriptKeywords,
    tsx: reactKeywords,
    test: testKeywords,
    sh: bashKeywords,
    yml: javascriptKeywords,
    ino: arduinoKeywords,
    py: pythonKeywords,
    mysql: mysqlKeywords,
  }[language];
...
async function displayContent() {
  ...
  if (sectionName !== "home") {
    ...
    const logoFileName = `images/image_name.jpg`;
    ...
    await displayRandomBhagavadGitaVerse(innerContainerElement);
  }
}
...
const mysqlKeywords = [
  ["CREATE", "DATABASE", "IF", "NOT", "EXISTS", "USE", "TABLE", "PRIMARY", "KEY"],
  ["INSERT", "INTO", "VALUES", "SELECT", "FROM", "WHERE", "ORDER", "BY", "UPDATE"],
  ["SET", "DELETE", "AUTO_INCREMENT", "VARCHAR", "NOT NULL", "UNIQUE", "DECIMAL", "DATE", "INT"]
];
...
const cppKeywords = [
  ["#include", "<iostream>", "<memory>", "template", "typename", "class", "struct", "private", "public", "std"],
  ["shared_ptr", "make_shared", "nullptr", "void", "const", "return", "if", "else", "while","int", "auto", "new", "delete"],
  ["pushBack", "pushFront", "popBack", "popFront", "backValue", "frontValue", "clear", "isEmpty", "size", "cin", "cout"]
];

const pythonKeywords = [
  ["class", "def", "self", "None", "is", "if", "else", "while", "return"],
  ["Node", "Deque", "push_back", "push_front", "pop_back", "pop_front", "back_value", "front_value", "clear"],
  ["__init__", "data", "front", "back", "next", "prev", "size", "print", "__main__"]
];
...
const javaKeywords = [
  ["import", "class", "public", "void", "static", "main", "String", "throws", "Exception"],
  ["@Test", "info", "default", "Retention", "Target", "ElementType", "Method", "annotation"],
  ["System.out.println", "invoke", "getDeclaredMethods", "isAnnotationPresent", "getAnnotation", "for", "if", "else"]
];
